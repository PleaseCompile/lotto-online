-- =============================================
-- Online Lottery Platform — Production Schema
-- PostgreSQL (Supabase-ready)
-- Version: 2.0 (aligned with application code)
-- =============================================
...
CREATE TABLE orders (
    ...
    status              varchar(20) NOT NULL DEFAULT 'pending_payment'
                        CHECK (status IN ('pending_payment', 'verifying', 'paid', 'cancelled', 'expired', 'refunded')),
...
CREATE TABLE prize_claims (
    ...
    status            varchar(20) NOT NULL DEFAULT 'pending'
                      CHECK (status IN ('pending', 'claimed', 'paid', 'rejected')),
    paid_at           timestamptz,
...
CREATE INDEX idx_claims_ticket ON prize_claims (ticket_id);
CREATE INDEX idx_claims_draw ON prize_claims (draw_id);
...
CREATE TABLE wallet_transactions (
    ...
    type            varchar(20) NOT NULL
                    CHECK (type IN ('deposit', 'withdrawal', 'prize', 'prize_payout', 'purchase', 'refund')),
    amount          integer NOT NULL,
...
CREATE TABLE notifications (
    ...
    type        varchar(30) NOT NULL
                CHECK (type IN ('order_paid', 'prize_won', 'prize_payout', 'prize', 'cart_expiring', 'system')),
...
-- Draw results: anyone can read
CREATE POLICY "draw_results_select_public" ON draw_results
    FOR SELECT USING (true);
ALTER TABLE draw_results ENABLE ROW LEVEL SECURITY;

-- Lottery draws: anyone can read
CREATE POLICY "lottery_draws_select_public" ON lottery_draws
    FOR SELECT USING (true);
